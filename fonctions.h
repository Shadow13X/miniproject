...
void e_lfa(int idad)
{
    ...
    else
    {
        ...
        while (fread(&e_tmp, sizeof(Emprunte), 1, f) != NULL) //searching in all the element of the file
        {
            if (e_tmp.id_ad==idad)
            {
                while (fread(&m_tmp, sizeof(Film), 1, _f) != NULL) //searching in all the element of the file
                {
                    if (m_tmp.code_film==e_tmp.id_film)
                    {
                        printf("%s.\n\n",m_tmp.titre);
                        i = 1;
                    }
...
{ //fonction pour modifier un film
    int n;
    FILE *_f,*f = NULL;          //pointeur de type file pour pointer sur fichier
...
{                                        //fonction pour supprimer un film
    ...
    if (!f)
    {
        printf("Unable to open file \"film\"");
    }
    else
    {
        FILE *p = fopen("tmp.bin", "wb");                 //creer un autre fichier pour
